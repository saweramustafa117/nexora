  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      {[
        ...
        <div key={title} className="flex flex-col gap-3 rounded-2xl border bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            <div className="shrink-0 rounded-lg bg-slate-100 p-2"><Icon className="h-5 w-5" /></div>
            <div><p className="font-medium">{title}</p><p className="text-xs text-slate-500">{desc}</p></div>
          </div>
          <button className="w-full rounded-lg border px-3 py-2 text-xs font-medium hover:bg-slate-50 sm:w-auto sm:py-1.5">Configure</button>
        </div>
      ))}
    </div>
  );
